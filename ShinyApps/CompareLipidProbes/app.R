...
# df <- read_csv(
#   "/Users/gaelenguzman/LipidInteractomics_Website_local/LipidInteractomics_Website/ShinyApps/CompareLipidProbes/combinedProbeDatasets_TMT.csv"
# ) |>
#   mutate(probeOptions = paste0(LipidProbe, " - ", CellLine))
#
# studies <- read_csv(here(
#   "/Users/gaelenguzman/LipidInteractomics_Website_local/LipidInteractomics_Website/ShinyApps/CompareLipidProbes/Studies.csv"
# ))
...
server <- function(input, output, session) {
  ...
  # Lookup study info for each probe
  output$probe1Info <- renderUI({
    row <- studies[studies$studyPointers == input$probe1, ]
    HTML(
      sprintf(
        'Selected Probe 1 (%s): <a href="%s" target="_blank">%s</a>',
        row$studyPointers,
        row$URL,
        row$Publication
      )
  ...
  output$probe2Info <- renderUI({
    row <- studies[studies$studyPointers == input$probe2, ]
    HTML(
      sprintf(
        'Selected Probe 2 (%s): <a href="%s" target="_blank">%s</a>',
        row$studyPointers,
        row$URL,
        row$Publication
      )
