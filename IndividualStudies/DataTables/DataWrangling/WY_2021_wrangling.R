...
filtered_data <- RawData |> 
  filter(if_all(starts_with("Control_"), ~ !is.na(.) & . != 0) & 
...
t_test_results <- filtered_data |> 
  group_by(gene_name) |> 
  summarise(
    pvalue = t.test(Abundance[Condition == "Sample"], 
                     Abundance[Condition == "Control"], 
                     var.equal = TRUE)$p.value,
    logFC = log2(mean(Abundance[Condition == "Sample"]) / mean(Abundance[Condition == "Control"]))
  ) |> 
  mutate(adj_p_value = p.adjust(pvalue, method = "BH"),
          LipidProbe = "PDAA",
          hit_annotation = case_when(.default = "no hit",
                                      logFC > log2(1.5) &  pvalue < 0.05 ~ "enriched hit"))  # Adjust p-values for multiple testing
