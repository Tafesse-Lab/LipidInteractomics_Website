...
dataRaw <- readr::read_csv(paste0(here(), "/IndividualStudies/DataTables/DataWrangling/AT_ChemComm2025_membrane.csv")) 

data <- dataRaw |>
  mutate(
    gene_name = Gene,
    protein_id = Protein.ID,
    hit_annotation = case_when(hit_annotation == "hit" & logFC < 0 ~ "no hit",
                               hit_annotation == "candidate" & logFC < 0 ~ "no hit",
                               .default = hit_annotation),
    LipidProbe = case_when(comparison.label == "PE_UV vs PE_NoUV" ~ "PE",
                           comparison.label == "PA_UV vs PA_NoUV" ~ "PA")
  ) |>
  dplyr::select(gene_name, protein_id, Protein.Description, LipidProbe, sample, logFC, AveExpr, pvalue, hit_annotation, -Gene, -Protein.ID, -comparison.label) |>
...
dataRaw <- readr::read_csv(paste0(here(), "/IndividualStudies/DataTables/DataWrangling/AT_ChemComm2025_cytosol.csv")) 
...
data <- dataRaw |>
  mutate(
    ...
    LipidProbe = case_when(comparison.label == "PE_UV vs PE_NoUV" ~ "PE",
                           comparison.label == "PA_UV vs PA_NoUV" ~ "PA"),
    sample = case_when(sample == "cytosole" ~ "cytosol",
                      .default = sample)
  ) |>
  dplyr::select(gene_name, protein_id, Protein.Description, LipidProbe, sample, logFC, AveExpr, pvalue, hit_annotation, -Gene, -Protein.ID, -comparison.label) |>
  glimpse()
