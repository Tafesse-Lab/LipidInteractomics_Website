...
AT_2025$hit_annotation <- factor(pull(AT_2025, hit_annotation), levels = c("enriched hit", "enriched candidate", "no hit", "NA"), ordered=TRUE)

# Building an ID column to maintain the order on the x-axis
ID <- c()
for(i in 0:(nrow(AT_2025)/2)){
	ID[i] = i
}

# Applies the ID column to the PE results
AT_2025_ranked_PE <- AT_2025 |>
    arrange(logFC) |>
	filter(LipidProbe == "PE") |>
	mutate(ID = ID) 

# Applies the ID column to the PA results
AT_2025_ranked_PA <- AT_2025 |>
    arrange(logFC) |>
...
VolcanoPlotStandardized(AT_2025)
...
AT_2025_reactive <- crosstalk::SharedData$new(AT_2025)
...
datatable(AT_2025_reactive, options = list(pageLength = 20))
