...
Gaelen Guzman\textsuperscript{1,}*, André Nadler\textsuperscript{3},
Frank Stein\textsuperscript{4}, Jeremy M. Baskin\textsuperscript{5},
Carsten Schulz\textsuperscript{2}, Fikadu G.
Tafesse\textsuperscript{1,2}

\textsuperscript{1}Department of Molecular Immunology \& Immunology,
Oregon Health \& Science University, Portland, OR, USA;
\textsuperscript{2}Chemical Physiology and Biochemistry, Oregon Health
\& Science University, Portland, OR, USA; \textsuperscript{3}Max Plank
Institute of Molecular Cell Biology and Genetics, Dresden Germany;
\textsuperscript{4}Proteomics Core Facility, European Molecular Biology
Laboratory, Heidelberg, Germany; \textsuperscript{5}Department of
Chemistry and Chemical Biology and Weill Institute for Cell and
Molecular Biology, Cornell University, Ithaca, NY, USA; *Corresponding
author:
...
and intracellular signaling{[}@barischMembraneDamageRepair2023;
@vanmeerMembraneLipidsWhere2008a;
@holthuisLipidLandscapesPipelines2014a;
@niphakisGlobalMapLipidBinding2015a;
@chernomordikProteinLipidInterplayFusion2003;
@simonsLipidRaftsSignal2000a{]}. Interactions with proteins are
essential to many of these crucial lipid functions -- and yet,
lipid--protein interactions are extremely challenging to study: The
small size and biophysical properties of lipids make conventional
proteomics approaches unsuitable for mapping lipid
interactors{[}@schultzChemicalToolsLipid2023{]}. Unlike proteins, which
can be readily tagged or immobilized for affinity-based interaction
studies, lipids require specialized chemical modifications to enable
selective and controlled interaction
...
interactomes{[}@schultzFlashClickMultifunctionalized2022a{]}. These
probes incorporate strategically designed functional groups that enable
covalent crosslinking, affinity enrichment, and controlled activation.
Most commonly, these include: (1) a diazirine group for photoactivatable
crosslinking upon exposure to 355 nm ultraviolet (UV) light, stabilizing
non-covalent and often modest-affinity lipid--protein interactions; (2)
a terminal alkyne, permitting click chemistry-based enrichment; and, in
some cases, (3) a coumarin photocage, which shields the lipid probe from
premature enzymatic modification until it is selectively cleaved by 405
nm light (a wavelength that does not induce diazirine
photolysis){[}@schultzFlashClickMultifunctionalized2022a;
@schultzChemicalToolsLipid2023; @hoglingerTrifunctionalLipidProbes2017;
@haberkantFatFabulousBifunctional2014{]}. Together, these modifications
enable direct proteomic identification of lipid interactors,
distinguishing true binding partners from transient or non-specific
associations{[}@haberkantFatFabulousBifunctional2014;
...
phosphatidyl alcohol{[}@hoglingerTrifunctionalLipidProbes2017;
@farleyTrifunctionalSphinganineNew2024a;
@farleyTrifunctionalFattyAcid2024;
@thomasTrifunctionalLipidDerivatives2025;
@chiuPhotoaffinityLabelingReveals2025;
@yuChemoproteomicsApproachProfile2022;
@mullerSynthesisCellularLabeling2021{]}. These initial studies have
followed a comparative proteomics workflow, wherein crosslinked
(``+UV'') and non-crosslinked (``-UV'') samples are analyzed side by
side to minimize artifacts and confirm
specificity{[}@haberkantFatFabulousBifunctional2014;
@schultzChemicalToolsLipid2023{]}. However, despite the growing number
...
cells{[}@thomasTrifunctionalLipidDerivatives2025;
@mullerSynthesisCellularLabeling2021;
@chiuPhotoaffinityLabelingReveals2025;
@yuChemoproteomicsApproachProfile2022;
@farleyTrifunctionalSphinganineNew2024a{]}. Additionally, it includes
control datasets from two fatty acid probes to distinguish specific from
non-specific lipid-binding
proteins{[}@farleyTrifunctionalFattyAcid2024{]}. We additionally intend
to expand this resource to include more complex experimental designs as
they are published (e.g.~Disease versus Healthy Control or Infection
versus Mock).
...
filtering{[}@plotly; @ramnathvaidyanathanHtmlwidgetsHTMLWidgets2015{]}.
For transparency and reproducibility, all datasets associated with a
given study are available for direct download.
...
comparisons~(\href{https://www.lipidinteractome.org/probevsprobecomparison/enrichedhitscomparison}{Probe~vs~Probe~Comparisons}).
...
Tagging{[}@thompsonTandemMassTags2003{]}. These datasets include those
reported by Farley et al.~(2024), Farley et al.~(2024), and Thomas et
al.~(2025){[}@farleyTrifunctionalSphinganineNew2024a;
@farleyTrifunctionalFattyAcid2024;
@thomasTrifunctionalLipidDerivatives2025{]}. Some studies have also
featured quantitative MS analysis using stable isotope labeling of amino
acids in cell culture (SILAC), which also enables extraction of key data
described below{[}@chiuPhotoaffinityLabelingReveals2025;
@yuChemoproteomicsApproachProfile2022{]}. Correspondingly, the ideal
data structure for incorporation into our repository will contain the
information shown in Table~\ref{tbl-dataStructure}.
...
identification.}\label{tbl-dataStructure}\tabularnewline
\toprule\noalign{}
...
easy incorporation and cross-study comparison. As proteomics techniques
have improved rapidly in recent years, we have done our best to
incorporate all data structures into this repository.}
...
(DDA; e.g.~TMT) {[}@birhanuMassSpectrometrybasedProteomics2023;
@boysClinicalApplicationsMass2023{]}; as with PSM datasets, we are
working to incorporate DIA data formats.

At present, we are accepting datasets as analyzed by the original
researchers, including protein identifications, enrichment
quantification, and the results of statistical testing. Summaries of
both source and data wrangling can be found in the \emph{Study
Information} dropdown section of each data page. Further, the raw data
(as provided directly by authors or pulled from publicly-available
datasts) and data wrangling scripts are available for download in the
same dropdown tabs.
...
interactomics. Ongoing efforts seek to incorporate enhanced scalability
of this site, more robust data storage methods, and automated data
submission. These functionalities will improve the utility of this
repository, enabling applications such as machine learning for pattern
recognition as the site grows. Future iterations will incorporate
validation studies for each lipid probe, offering researchers a
dedicated space to compare findings across experimental contexts. This
functionality will not only reduce redundancy in experimental design but
also strengthen the reproducibility and biological interpretation of
lipid interactome research.

To support this vision, we invite researchers to contribute their
datasets via our
\href{https://lipidinteractome.org/contactus/datasubmission}{Data
Submission page}. Using the FAIR data principles, we hope to foster a
collaborative and transparent approach to lipid biology.
...
GG designed, implemented, and wrote the majority of the Lipid
Interactome repository and manuscript. FS wrote the data analysis
section of the repository, provided significant intellectual
contribution to the repository design, and edited the manuscript. AN
provided substantial editing of the repository and manuscript. JMB
edited the manuscript and contributed data to the repository. CS and FT
provided significant intellectual contribution and text editing of the
repository and manuscript and oversaw the site development.
