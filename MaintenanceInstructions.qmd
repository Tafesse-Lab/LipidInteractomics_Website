...
If your data looks at all like [Alix's](IndividualStudies/DataTables/AT_ChemicalCommunications_2025.csv){target="_blank"} or [Scotty's](IndividualStudies/DataTables/SF_ACS-Chem-Bio_2024_download.csv){target="_blank"}, the ggplot functions I built in the ggplot_styles.R file will apply. Call the RankedOrderPlotStandard() and VolcanoPlotStandardized() functions to produce these plots with the same standardized formatting. On that note, you can alter the ggplot_styles.R file to change the standardized formatting. 
